...
struct Tournament {
    ...
    void create_matchups() {
        ...
        fmt::print("Creating all combination matches....\n");//we just create the possible matches we have, we will need to evaluate the points in each day.
        ...
        for(u_int64_t day = 0; day < days; day++){
            ...
            std::sort(games.begin(), games.end(), [day, players_day](const Game &x, const Game &y){//sorting by less points per match
                if((players_day[x.white].hasRested || players_day[x.black].hasRested)&&(!players_day[y.white].hasRested || !players_day[y.black].hasRested)){
                    return true;
                }
                if((players_day[y.white].hasRested || players_day[y.black].hasRested)&&(!players_day[x.white].hasRested || !players_day[x.black].hasRested)){
                    return false;
                }
                return players_day[(uint64_t) x.white].points_per_day[day] + players_day[(uint64_t) x.black].points_per_day[day] < players_day[(uint64_t) y.white].points_per_day[day] + players_day[(uint64_t) y.black].points_per_day[day];
            });
            ...
            }else fmt::print("DAY {} COMPLETED\n", day);
                
            
        }
    }

    bool find_best_match(std::vector<Game> &games, std::vector<Game> &gamesPlayed, std::vector<uint64_t> &todayPlayer, uint64_t day){
