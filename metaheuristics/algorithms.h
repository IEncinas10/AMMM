...
static bool useLocalSearch = false;

// whether to print or not the full calendar
static bool print_calendar = true;

// hack to print nicely the tournament schedule
static std::size_t NUM_PLAYERS;
...
struct Tournament {
	...
	// Generate set of valid games (just for validating solution, this does nothing right else
	// now)
	create_games();
	...
	if (print_info) {
	    fmt::print("Fake rest: [{:{}}]\nRest:      [{:{}}]\n", fmt::join(fakerest, " "),
		       id_width, fmt::join(rests, " "), id_width);
	}
	...
	for (uint32_t day = 0; day < days; day++) {
		...
		uint32_t rename_index_w =
		    std::find(fakerest.begin(), fakerest.end(), w) - fakerest.begin();
		uint32_t rename_index_b =
		    std::find(fakerest.begin(), fakerest.end(), b) - fakerest.begin();
	...
	// Additionally, check that every player plays a correct amount of games and they're 50%w,
	// 50%b
	...
	do {
		...
		for (uint32_t j = 0; j < num_days; j++) {
		    auto curr_points =
			players[rests[day]].points[day] + players[rests[j]].points[j];
		    auto swap_points =
			players[rests[j]].points[day] + players[rests[day]].points[j];

		    int change = swap_points - curr_points;
		    if (change > best_swap_points) {
			// fmt::print("Swapping {} and {}. Change: {}. Prev: {}\n", rests[day],
			// rests[j], change, best_swap_points);
			best_swap = j;
    ...
    void assign_rest(std::vector<Player> &players_day, uint64_t day, std::vector<player_id> &rests,
		     uint64_t &score) {
	...
	for (const auto &p : players) {
		...
		fmt::print("Player {} has played {} games. {} w, {} b\n", p.playerID,
			   p.games_played, p.games_white, p.games_black);
	}
    }
};
...
bool read_instance(const char *instance_filename, Tournament &tournament) {
    ...
    uint64_t lines_read = 0, number_of_players = 0, remaining_players = UINT64_MAX,
	     player_index = 0;
...
template <> struct fmt::formatter<Match> {
	...
	uint8_t id_width = std::log10(NUM_PLAYERS) + 1;
	return fmt::format_to(ctx.out(), "[Day: {0:{3}}, White: {1:{3}}, Black: {2:{3}}]", m.day,
			      m.white, m.black, id_width);
    }
};
