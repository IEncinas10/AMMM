...
For tuning $\alpha$, we have calculated the error of the resulting GRASP's objective function with respect to the optimal solution for each of the instances. Then, for each $\alpha$ we calculated the arithmetic mean error. 
...
Using that data, we can say (taking in account the fine-grained tuned alpha) that $\alpha = 0.19$ is the best and lowest alpha we find to obtain the best mean error for the instances 3 to 31.
