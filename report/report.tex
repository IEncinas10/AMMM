...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\For {$day\ in\ days$}
      \State playersToRest $\leftarrow$ filter sortedPlayers(p) | p.hasNotRested
      \State sortedPlayers $\leftarrow$ sort PlayersToRest by points per day (DESC) 
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\For {$i\ =\ 1,2,...,Nplayers$}
      \For {$j\ =\ 1,2,...,Nplayers$}
        \State change = EvaluateSwapRest(i,j)
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
      ...
      \State playersToRest $\leftarrow$ filter sortedPlayers(p) | p.hasNotRested
      \State sortedPlayers $\leftarrow$ sort PlayersToRest by points per day (DESC) 
      \State last\_index $\leftarrow$ (playersToRest.size - 1) * $\alpha$
      \State RCL $\leftarrow$ sortedPlayers(p) | p $\in$ [0, last\_index] 
